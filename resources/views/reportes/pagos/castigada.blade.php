{{-- resources/views/reportes/pagos/castigada.blade.php --}}
...
<style>
  .rpt-castigada .table thead th{
    position: sticky; top: 0; z-index: 1;
    background: color-mix(in oklab, var(--surface-2) 55%, transparent);
    color: var(--ink);
  }
  [data-theme="dark"] .rpt-castigada .table thead th{
    background: color-mix(in oklab, var(--surface-2) 40%, transparent);
  }
  .rpt-castigada .table tbody td{ color: var(--ink) }
  .rpt-castigada .table tbody tr:nth-child(even){
    background: color-mix(in oklab, var(--surface-2) 22%, transparent);
  }
  .rpt-castigada .table tbody tr:hover{
    background: color-mix(in oklab, var(--brand) 10%, transparent);
  }

  /* Paginación sin el azul de Bootstrap */
  .rpt-castigada .pagination{ margin-bottom: 0 }
  .rpt-castigada .page-link{
    border-color: var(--border);
    background: var(--surface);
    color: var(--ink);
  }
  .rpt-castigada .page-link:hover{
    background: color-mix(in oklab, var(--brand) 10%, transparent);
    border-color: color-mix(in oklab, var(--brand) 30%, transparent);
    color: var(--ink);
  }
  .rpt-castigada .page-item.active .page-link{
    background: var(--brand);
    border-color: var(--brand);
    color: #fff;
  }
  .rpt-castigada .page-item.disabled .page-link{
    color: var(--muted);
    background: var(--surface);
  }

  .rpt-castigada .pager-meta{ color: var(--muted) }
</style>

<div class="rpt-castigada">
  {{-- Tabla: Caja Cusco ▸ Castigada (10 por página) --}}
