...
@if($ppPendCount || $cnaPendCount || $vencCount)
  <div class="alert alert-warning d-flex align-items-center justify-content-between">
    <div>
      ...
      @if($ppPendCount)
        <span class="ms-2 badge text-bg-warning">{{ $ppPendCount }}</span>
        promesa(s) pendiente(s) @if($isSupervisor) de pre-aprobar @else de aprobar @endif
      @endif
      @if($cnaPendCount)
        <span class="ms-3 badge text-bg-info">{{ $cnaPendCount }}</span> CNA(s) pendientes
      @endif
      @if($vencCount)
        <span class="ms-3 badge text-bg-danger">{{ $vencCount }}</span> cuota(s) vencen en 7 días
      @endif
    </div>
...
<div class="row g-3 mt-1">
  ...
  <div class="col-lg-6">
    <div class="card h-100">
      ...
      <div class="card-footer text-end">
        <a href="{{ route('autorizacion') }}" class="btn btn-sm btn-outline-primary">Ir a Autorización</a>
      </div>
    </div>
  </div>
