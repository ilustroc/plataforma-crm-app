...
<div x-data="autorizacion" class="max-w-7xl mx-auto">
    ...
    <div class="auth-card">
        <div class="auth-header">
            ...
            <form method="GET" action="{{ route('autorizacion') }}" class="flex gap-2 w-full sm:w-auto">
                ...
                <button type="submit" class="btn-action btn-primary px-4 shadow-lg shadow-brand/20">Buscar</button>
                @if($q)
                ...
                <tbody>
                    @forelse($rows as $p)
                        @php
                            ...
                            // Data segura para Alpine (no usar {{ json_encode(...) }} en @click)
                            $data = [
                                'dni'     => $p->dni,
                        ...
                        <tr class="group hover:bg-slate-50/50">
                            ...
                            <td class="text-right">
                                <div class="flex justify-end gap-1">
                                    {{-- Botón Ver Ficha --}}
                                    <button type="button"
                                            class="btn-action btn-outline h-7 px-2"
                                            @click.prevent='openFicha(@json($data))'
                                            title="Ver Detalle">
                                    ...
                                    @if($isSupervisor)
                                        {{-- Supervisor --}}
                                        <button type="button"
                                                @click.prevent="openNota('{{ route('autorizacion.preaprobar', $p) }}', 'Pre-aprobar Promesa')"
                                                class="btn-action btn-primary h-7 px-2 bg-emerald-600 hover:bg-emerald-700 border-transparent text-white"
                                                title="Pre-aprobar">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <button type="button"
                                                @click.prevent="openRechazo('{{ route('autorizacion.rechazar.sup', $p) }}')"
                                                class="btn-action btn-danger-outline h-7 px-2 text-rose-600 border-rose-200 hover:bg-rose-50"
                                                title="Rechazar">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @else
                                        {{-- Admin --}}
                                        <button type="button"
                                                @click.prevent="openNota('{{ route('autorizacion.aprobar', $p) }}', 'Aprobar Promesa')"
                                                class="btn-action btn-primary h-7 px-2 bg-brand hover:bg-brand/90 border-transparent text-white"
                                                title="Aprobar">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                        ...
                                        <button type="button"
                                                @click.prevent="openRechazo('{{ route('autorizacion.rechazar.admin', $p) }}')"
                                                class="btn-action btn-danger-outline h-7 px-2 text-rose-600 border-rose-200 hover:bg-rose-50"
                                                title="Rechazar">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
    ...
    {{-- MODAL FICHA DETALLE --}}
    <div x-show="modalFichaOpen"
         x-cloak
         class="modal-backdrop"
         :class="modalFichaOpen ? 'show' : ''">
        <div class="modal-box" @click.outside="modalFichaOpen = false">
            <div class="modal-header">
                <h3 class="font-bold text-lg text-slate-800">Detalle de Propuesta</h3>
                <button type="button" @click="modalFichaOpen = false" class="text-slate-400 hover:text-slate-600">✕</button>
            </div>

            <div class="modal-body space-y-4">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <div>
                        <span class="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Cliente</span>
                        <span class="font-bold text-slate-700" x-text="ficha.nombre"></span>
                    </div>
                    <div>
                        <span class="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">DNI</span>
                        <span class="font-mono text-slate-600" x-text="ficha.dni"></span>
                    </div>
                    <div>
                        <span class="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Tipo</span>
                        <span class="uppercase badge badge-info" x-text="ficha.tipo"></span>
                    </div>
                    <div>
                        <span class="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Monto</span>
                        <span class="font-bold text-brand" x-text="fmtMoney(ficha.monto)"></span>
                    </div>
                </div>
                        ...
                        <table class="w-full text-xs text-left">
                            <thead class="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="px-4 py-2.5">Entidad / Producto</th>
                                    <th class="px-4 py-2.5 text-right">Deuda Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                <template x-for="c in ficha.cuentas" :key="c.operacion">
                                    <tr class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                                        ...
                                        <td class="px-4 py-2 text-slate-600">
                                            <span x-text="c.entidad"></span>
                                            -
                                            <span class="text-slate-400" x-text="c.producto"></span>
                                        </td>
                    ...
                    <div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm max-h-40 overflow-y-auto">
                        <table class="w-full text-xs text-left">
                            <thead class="bg-slate-50 text-slate-500 font-bold border-b border-slate-200 sticky top-0">
                                <tr>
                                    <th class="px-4 py-2.5 text-center">Cuota</th>
            ...
            <div class="modal-footer">
                <button type="button" @click="modalFichaOpen = false" class="btn-action btn-outline w-full sm:w-auto">Cerrar</button>
            </div>
        </div>
    </div>

    {{-- MODAL APROBACIÓN (Nota) --}}
    <div x-show="modalNotaOpen"
         x-cloak
         class="modal-backdrop"
         :class="modalNotaOpen ? 'show' : ''">
        <div class="modal-box max-w-md" @click.outside="modalNotaOpen = false">
    ...
    <div x-show="modalRechazoOpen"
         x-cloak
         class="modal-backdrop"
         :class="modalRechazoOpen ? 'show' : ''">
        <div class="modal-box max-w-md" @click.outside="modalRechazoOpen = false">
            <form :action="actionUrl" method="POST">
                @csrf
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <textarea name="nota_estado" id="txtRechazo" rows="3"
                              class="form-input border-red-200 focus:border-red-500 bg-red-50/30"
                              required></textarea>
                </div>
